...
export default function Index() {
  const answerKey = process.env.NEXT_PUBLIC_JMC12_KEY;
  ...
  function Submit() {
    ...
    fetch("/api/submit", {
      method: "POST",
      body: JSON.stringify({ version: 12, username, answers, score }),
    });
    return;
  }
  function ShowUsername(show) {
    fetch("/api/showUsername", {
      method: "POST",
      body: JSON.stringify({ version: 12, show, username }),
    });
  }
  ...
  return (
    <div className="mx-8 my-8 space-y-2">
      <h1>JMC 12 Submission Portal</h1>
      <h2>Instructions</h2>
      <p>
